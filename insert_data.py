...
from add_data import insert_course
...
def add_courses_to_programme(db_connector, cursor, programme_id: int, courses: list[tuple[str, str, int, int, int, int, str]]) -> None:
    """
    Adds multiple courses to a programme and links them to the programme.

    Each tuple in `courses` is:
    (code, name, credits, L, P, T, department)
    """
    for code, name, credits, L, P, T, department in courses:
        cursor.execute("""INSERT INTO courses (code, name, credits, L, P, T, department_id)
                          VALUES (%s, %s, %s, %s, %s, %s,
                          (SELECT id FROM departments WHERE name=%s))
                          ON DUPLICATE KEY UPDATE name=VALUES(name), credits=VALUES(credits),
                          L=VALUES(L), P=VALUES(P), T=VALUES(T), department_id=VALUES(department_id);""",
                       (code, name, credits, L, P, T, department))
        db_connector.commit()

        cursor.execute("INSERT IGNORE INTO programme_courses (programme_id, course_code) VALUES (%s, %s);",
                       (programme_id, code))
        db_connector.commit()
