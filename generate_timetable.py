...
# fetch_data aligned with actual schema including section_class mapping
class fetch_data:
    @staticmethod
    def get_sections(cursor: Cursor, campus_id: Optional[int] = None) -> List[Dict]:
        # stream nullable, join programmes
        query = (
            "SELECT s.id, s.campus_id, p.degree, p.stream, s.section, s.year "
            "FROM sections s "
            "JOIN programmes p ON s.programme_id = p.id"
        )
        if campus_id is not None:
            query += " WHERE s.campus_id = %s"
    ...
    @staticmethod
    def get_section_student_count(cursor: Cursor, section_id: int) -> int:
        cursor.execute(
            "SELECT COUNT(*) AS cnt FROM section_students WHERE section_id = %s",
            (section_id,)
        )
        return cursor.fetchone()['cnt']

    @staticmethod
    def get_group_student_count(cursor: Cursor, section_ids: List[int]) -> int:
        placeholder = ','.join(['%s'] * len(section_ids))
        cursor.execute(
            f"SELECT COUNT(*) AS cnt FROM section_students WHERE section_id IN ({placeholder})",
            tuple(section_ids)
        )
        return cursor.fetchone()['cnt']

    @staticmethod
    def get_section_class(cursor: Cursor) -> Dict[int, int]:
        # mapping section_id -> class_id
        cursor.execute("SELECT section_id, class_id FROM section_class")
        return {row['section_id']: row['class_id'] for row in cursor.fetchall()}

    @staticmethod
    def get_faculty_sections(cursor: Cursor) -> List[Dict]:
        # include course hours L, T, P
        cursor.execute(
            """
            SELECT fsc.id, fsc.faculty_id, fsc.section_id, fsc.course_code,
                   c.L, c.T, c.P
            FROM faculty_section_course fsc
            JOIN courses c ON fsc.course_code = c.code
            """
        )
        return cursor.fetchall()

    @staticmethod
    def get_classes(cursor: Cursor, campus_id: Optional[int] = None, is_lab: bool = False) -> List[Dict]:
        query = (
            "SELECT cl.id, cl.capacity "
            "FROM classes cl "
            "JOIN campus_buildings cb ON cl.building_id = cb.building_id "
            "WHERE cl.is_lab = %s"
        )
        params: Tuple = (is_lab,)
        if campus_id is not None:
            query += " AND cb.campus_id = %s"
            params = (is_lab, campus_id)
        cursor.execute(query, params)
        return cursor.fetchall()

# Helper to insert into timetables table

def insert_into_timetable(cursor: Cursor, db_connector: Connection,
                          day: str, period_id: int,
                          fsc_id: int, class_id: int) -> None:
    try:
        cursor.execute(
            """
            INSERT INTO timetables (day, period_id, faculty_section_course_id, class_id)
            VALUES (%s, %s, %s, %s)
            """,
            (day, period_id, fsc_id, class_id)
        )
        db_connector.commit()
    except Exception:
...
def generate_timetable(db_connector: Connection,
	...
	for (day, periods) in minor_hrs.items():
